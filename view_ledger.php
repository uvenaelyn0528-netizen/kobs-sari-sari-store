...
// Direktang kunin ang pangalan gamit ang napatunayan nating query
$customer_name = '';
try {
    $stmt_name = $pdo->prepare("SELECT customer_name FROM customers WHERE id = ? LIMIT 1");
    $stmt_name->execute([$customer_id]);
...
// Fallback kung sakaling blangko
