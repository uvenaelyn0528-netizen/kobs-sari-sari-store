...
if (!isset($_SESSION['role']) || !in_array($_SESSION['role'], ['admin', 'tindera', 'viewer<?php
session_start();
require_once 'db.php';
                ...
                foreach ($rows as $raw_row) {
                    ...
                    foreach ($row as $col_key => $val) {
                        if (strpos($col_key, 'qty') !== false || strpos($col_key, 'quantity') !== false || strpos($col_key, 'count') !== false || strpos($col_key, 'out') !== false || strpos($col_key, 'sold') !== false) {
                            $clean_val = preg_replace('/[^0-9.-]/', '', (string)$val);
                            if ($clean_val !== '' && is_numeric($clean_val)) {
                                $qty = (int)$clean_val;
                                break;
                            }
                        }
                    ...
                    foreach ($row as $col_key => $val) {
                        ...
                        if (strpos($col_key, 'code') !== false || strpos($col_key, 'barcode') !== false) {
                            $c_code = preg_replace('/\s+/', '', $val);
                            $stockout_totals_by_code[$c_code] = ($stockout_totals_by_code[$c_code] ?? 0) + $qty;
                        }

                        if (strpos($col_key, 'name') !== false || strpos($col_key, 'product') !== false || strpos($col_key, 'description') !== false || strpos($col_key, 'item') !== false) {
                            $c_name = strtolower(trim($val));
                            $stockout_totals_by_name[$c_name] = ($stockout_totals_by_name[$c_name] ?? 0) + $qty;
                        }
                    }
                }
            }
